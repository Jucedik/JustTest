...
public class RssReader implements OnFeedLoadListener {
    ...
    private static String ENCLOSURE = "enclosure";
    ...
    public List<RssItem> parseSync() {
        if (mRssItems != null) {
            mRssItems.clear();
        }

        if (mUrlList != null) {
           return parseRssSync(0);
        }
        else {
            return null;
        }
    ...
    private List<RssItem> parseRssSync(int position) {
        if (position != mUrlList.length) {
            try {
                Document rssDocument = Jsoup.connect(mUrlList[position]).ignoreContentType(true).parser(Parser.xmlParser()).get();
    ...
    public void parseProviderInfo(OnRssLoadListener onRssLoadListener) {
        this.mOnRssLoadListener = onRssLoadListener;
        ...
        if (mUrlList != null) {
            parseProviderInfoImpl();
        } else {
            throw new NullPointerException("Url list cannot be empty");
        }
    }

    private void parseProviderInfoImpl() {
        mRssParser = new RssParser(mUrlList[0], this, true);
        mRssParser.execute();
        String source = getWebsiteName(mUrlList[0]);
    ...
    private void parseRss(int position) {
        if (position != mUrlList.length) {
            mRssParser = new RssParser(mUrlList[position], this, false);
            mRssParser.execute();
    ...
    public void onSuccessProviderInfo(String title, String description, String imageUrl) {
        ...
        mOnRssLoadListener.onSuccessProviderInfo(title, description, imageUrl);
    }
    ...
    private RssItem getRssItem(Element element) {
        ...
        if (!element.select(MEDIA_THUMBNAIL).isEmpty()) {
            imageUrl = element.select(MEDIA_THUMBNAIL).last().attr(URL);
        }
        else if (!element.select(MEDIA_CONTENT).isEmpty()) {
            imageUrl = element.select(MEDIA_CONTENT).last().attr(URL);
        }
        else if (!element.select(IMAGE).isEmpty()) {
            imageUrl = element.select(IMAGE).last().attr(URL);
        }
        else if (!element.select(ENCLOSURE).isEmpty()) {
            imageUrl = element.select(ENCLOSURE).last().attr(URL);
        }
        else {
            imageUrl = null;
        }
